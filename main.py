...
def get_predicted_value(patient_symptoms):
    ...
    for item in patient_symptoms:
        symptom = item.strip().lower().replace(" ", "_")
    ...
    if unknown_symptoms:
        print(f"⚠️ Unknown symptoms ignored: {', '.join(unknown_symptoms)}")
...
def contact():
    if request.method == 'POST':
        ...
        # You can print or save these details as needed
        print(f"New contact message from {name}: {message}")
